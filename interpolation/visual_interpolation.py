...
def plot_interpolation(data, delt=1):
    ...
    plot.scatter(intr_point[0], intr_point[1], color="red")
    plot.scatter(extr_point[0], extr_point[1], color="red")
    show_graph(x, y)
...
def plot_peacemeal(data):
    ...
    plot.scatter(m.matrix_transposition(in_coord)[
                 0], m.matrix_transposition(in_coord)[1], color="grey")
    plot.scatter(m.matrix_transposition(out_coord)[
                 0], m.matrix_transposition(out_coord)[1], color="grey")
    show_graph(x, y)
...
def plot_lagrange_polynom(data, delt=1):
    ...
    x = [i / 10 for i in range((min(arr_x) - delt)
                               * 10, (max(arr_x) + delt) * 10 + 1)]
